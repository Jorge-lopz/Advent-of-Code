...
with open('input.txt', 'r') as f:
    memory = list(map(int, f.read().strip()))

# FIRST PART: Sort the disk visual representation to leave all the free space at the end and compress the 'fs'
j = 0
disk = []
...
# SECOND PART: Similar to the first part but only moving whole fs together (using imaginary numbers like "tuples")
...
while right > 0:
    ...
    while disk[right].imag == 0:  # Find the next f from the right
        right -= 1

    # Find the first free space, where the entire f can fit
    while left < len(disk) and not (disk[left].imag == 0 and disk[left].real >= disk[right].real):
        left += 1

    if left < len(disk) and left < right:  # Switch the f with the free space
